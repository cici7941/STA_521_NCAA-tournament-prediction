...
fit.bayes <- bayesglm(datanewS[,30]~.,data=as.data.frame(datanewS[,1:29]), family = 'binomial', scaled = FALSE)
...
firstwinner1 <- firstround.winners(as.character(firstround), regSeason2014D,seed,lm1)
#predict firstround of 2014 tournament competition with model Elastic
firstwinner2 <- firstround.winners(as.character(firstround), regSeason2014D,seed,lm2)
#predict firstround of 2014 tournament competition with model ridge regression
firstwinner3 <- firstround.winners(as.character(firstround), regSeason2014D,seed,lm3)
#predict firstround of 2014 tournament competition with Bayesian GLM with stepwise
firstwinner4 <- firstround.winners(as.character(firstround), regSeason2014D,seed,lm4)
...
nextround.winners <- function(lastwinner, data, model){
  seed <- matrix(as.matrix(lastwinner)[,c(1,3,5,7)])
  id <- matrix(as.matrix(lastwinner)[,c(2,4,6,8)])
  win <- 0; game <- 0
  seedWinner <- 0
  k <- seq(2,nrow(id),2)
  for(i in k){
  ...
  winner <- data.frame(Wseed=seedWinner[1:(length(win)/4)],Wwinner=win[1:(length(win)/4)],
                       Xseed=seedWinner[(length(win)/4+1):(length(win)/2)],
                       Xwinner=win[(length(win)/4+1):(length(win)/2)],
                       Yseed=seedWinner[(length(win)/2+1):(length(win)*3/4)],
                       Ywinner=win[(length(win)/2+1):(length(win)*3/4)],
                       Zseed=seedWinner[(length(win)*3/4+1):length(win)],
                       Zwinner=win[(length(win)*3/4+1):length(win)])
  return(winner)
}
##second round result
w1 <- nextround.winners(firstwinner1,regSeason2014D,lm1)
w2 <- nextround.winners(firstwinner2,regSeason2014D,lm2)
w3 <- nextround.winners(firstwinner3,regSeason2014D,lm3)
w4 <- nextround.winners(firstwinner4,regSeason2014D,lm4)
##third round result
w1n <- nextround.winners(w1,regSeason2014D,lm1)
w2n <- nextround.winners(w2,regSeason2014D,lm2)
w3n <- nextround.winners(w3,regSeason2014D,lm3)
w4n <- nextround.winners(w4,regSeason2014D,lm4)
##final four
lasso <- nextround.winners(w1n,regSeason2014D,lm1)
elas <- nextround.winners(w2n,regSeason2014D,lm2)
ridge <- nextround.winners(w3n,regSeason2014D,lm3)
bayes <- nextround.winners(w4n,regSeason2014D,lm4)
...
```{r}
final <- function(lastwinner, data, model){
  game1 <- t(average(data, matrix(lastwinner$Wwinner), matrix(lastwinner$Xwinner)))
  game2 <- t(average(data, matrix(lastwinner$Ywinner), matrix(lastwinner$Zwinner)))
  if (class(model)[1]=="bayesglm"){
    game1 = as.data.frame(game1)
    game2 = as.data.frame(game2)
  }
  win1 <- predict(model, game1, type = "response")
  win2 <- predict(model, game2, type = "response")
  if(win1 >= 0.5){
    win1 <- lastwinner$Wwinner
    seedWinner1 <- lastwinner$Wseed
  }else{
    win1 <- lastwinner$Xwinner
    seedWinner1 <- lastwinner$Xseed
  }
  if(win2 >= 0.5){
    win2 <- lastwinner$Ywinner
    seedWinner2 <- lastwinner$Yseed
  }else{
    win2 <- lastwinner$Xwinner
    seedWinner2 <- lastwinner$Zseed
  }
  semiwinner <- data.frame(seed = c(levels(seedWinner1),levels(seedWinner2)), 
                           winteam = c(levels(win1),levels(win2)))
  print(semiwinner)
  game3 <- t(average(data, matrix(semiwinner$winteam[1]), matrix(semiwinner$winteam[2])))
  if (class(model)[1]=="bayesglm"){
    game3 = as.data.frame(game1)
  }
  win3 <- predict(model, game3, type = "response")
  if(win3 >= 0.5){
    win3 <- semiwinner$winteam[1]
    seedWinner3 <- semiwinner$seed[1]
  }else{
    win3 <- semiwinner$winteam[2]
    seedWinner3 <- semiwinner$seed[2]
  }
  champion <- data.frame(seed = levels(seedWinner3)[1], winteam = levels(win3)[1])
  print(champion)
  return(champion)
}